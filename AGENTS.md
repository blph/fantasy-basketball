...
One exception: the **2026-27 draft board** is built and in use. It is a Google Sheet generated by [scripts/draft-board/](scripts/draft-board/) from manual provider exports, standing in for the pipeline until Phase 2 lands ([ADR-0008](docs/decisions/ADR-0008-google-sheet-draft-board.md)). New exports go in through `Refresh data`, which writes only the cells that changed and preserves hand-edited columns — see [docs/draft-board/](docs/draft-board/build-and-maintenance.md).
