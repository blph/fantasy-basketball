...
One exception: the **2026-27 draft board** is built and in use. It is a Google Sheet generated by [scripts/draft-board/](scripts/draft-board/) from manual provider exports, standing in for the pipeline until Phase 2 lands ([ADR-0008](docs/decisions/ADR-0008-google-sheet-draft-board.md)). New exports go in through `Refresh data`, which writes only the cells that changed and preserves hand-edited columns — see [docs/draft-board/](docs/draft-board/build-and-maintenance.md). The board is read back out through `playwright-cli` (a signed-in browser profile, not the Sheets API); `export_yahoo_rankings.py` turns that into a Yahoo import CSV. The same `build_data.py` run writes a **local copy** for agents: a JSON snapshot under `data/draft-board/`, read only through `board.py`. Its engine reproduces the sheet's draft-day formulas, and `verify.py --local` holds it to the live sheet ([ADR-0022](docs/decisions/ADR-0022-local-draft-board.md)).

The board runs on **three projection sources** (BMP, HBP, BMP-ALT) and **Basketball Monster's DURANT H2H**, not the playbook's G-score ([ADR-0014](docs/decisions/ADR-0014-three-projection-sources.md), [ADR-0015](docs/decisions/ADR-0015-durant-h2h-primary-value.md)). Values are computed by `build_data.py` over `scripts/bbm/bbm_reference.py` and written to the sheet as **numbers**; only the draft-day logic stays a live formula ([ADR-0016](docs/decisions/ADR-0016-values-computed-in-python.md)). The two Basketball Monster sources do not derive their own standardisation constants — they borrow Basketball Monster's, recovered from its published columns by `calibrate_bbm.py` and refitted on every refresh ([ADR-0021](docs/decisions/ADR-0021-borrowed-bbm-pool-constants.md)). A refresh means **six** same-dated files moving together: three exports you download, and two fits plus Basketball Monster's injury-risk table, all three written by `calibrate_bbm.py`.
...
- Recover Basketball Monster's constants for a vendor source (do this before building): `python3 scripts/draft-board/calibrate_bbm.py --source BMP --date YYYY-MM-DD` (and again for `BMP-ALT`, whose run also writes the injury-risk table)
...
- Run the tick scenario on a throwaway copy: `python3 scripts/draft-board/tick_scenario.py --allow-consent` (copies the live sheet, ticks, punts, re-sorts and refreshes it, pulls and verifies after every step, deletes the copy; one line per step; ask Bryan before passing `--allow-consent`, which clicks Allow on the copy's authorisation screen; skips itself unless `Build.gs` or `board_engine.py` changed since the last pass, `--force` overrides)
...
- DO NOT compute, score, estimate or infer an injury tier, and do not reintroduce a durability heuristic anywhere in this repo. The `INJ` column is Basketball Monster's own `Inj Risk`, copied verbatim from `BBM Injury Risk - <date>.csv`, in their vocabulary: `EXTREME` / `HIGH` / `MED` / `LOW`. We tried scoring it ourselves from cited injury history and the rubric graded Jayson Tatum `MED` months after Achilles surgery; the column is theirs now and reading it is the whole job.
- DO NOT take the injury tier from a source other than `INJURY_SOURCE` in `calibrate_bbm.py` (currently `BMP-ALT`, their "Bonus" source). Their grades are per-source and disagree sharply: on 2026-09-12 Josh graded 3 of 234 players `low` and Tatum `med`, Bonus graded 54 `low` and Tatum `high`. It is deliberately not the source the board ranks on — the tier scales no value (ADR-0017), so it has nothing to be consistent with.
- DO NOT leave a board player's injury cell blank. A blank in a risk column reads as `LOW`; a player Basketball Monster does not grade renders `?`. Never guess a tier to fill the hole, and never hard-fail the build over one — the column scales nothing (ADR-0017). Use `build_data.py --require-injuries` when you want it fatal.
...
- DO NOT read from or write to the draft Google Sheet by any route except **Playwright**. Not the Sheets API, not the `google-sheets` MCP, not a service account, not `clasp`, not Drive's file-content reader. `playwright-cli` drives the owner's own signed-in Chrome profile, so it needs no sharing change and no extra credential, and it sees the sheet exactly as the owner does. The reason is **not** that the Sheets API is too weak to draw the board — v4 `batchUpdate` does support `AddNamedRange`, `AddConditionalFormatRule` and `SetDataValidation`, and this line used to claim otherwise, wrongly. The reason is that the board is an **Apps Script program**, not a grid: `Refresh data` is what writes only the changed cells and preserves the hand-edited columns, and no spreadsheet API can install or run it. Doing that by API means the *Apps Script* API — a standard Cloud project shared with the script, an API-executable deployment, and an OAuth token covering every scope the script uses — a new runtime dependency, and one that needs an ADR. The configured MCP is weaker still: values-only, and it authenticates as a service account this sheet is not shared with. Drive's file-content reader truncates the tab around rank 77 of 200 without saying so. Commands in [build-and-maintenance.md](docs/draft-board/build-and-maintenance.md); all four routes evaluated in full in [the route evaluation](docs/reviews/2026-09-12-sheet-access-route-evaluation.md).
...
Deeper docs: [data providers](docs/api/data-providers.md) · [database schema](docs/database/schema.md) · [decisions](docs/decisions/decision-log.md) · [bugs](docs/bugs/) · [draft board](docs/draft-board/build-and-maintenance.md) · [board cheat sheet](docs/draft-board/cheat-sheet.md) · [mock draft review](docs/draft-board/mock-draft-review.md) · [sheet access routes](docs/reviews/2026-09-12-sheet-access-route-evaluation.md) · [draft playbook](docs/references/fantasy-basketball-draft-playbook.md) · [quant vs expert](docs/references/quant-vs-expert-reconciliation.md) · [Basketball Monster reverse-engineering](docs/references/basketball-monster-projections-reverse-engineering.md) · [project updates](docs/project-updates/)
